...
class DatabaseHelper {
  ...
  Future<List<Map<String, dynamic>>> searchWargaByNama(String keyword) async {
    final db = await database;
    return await db.query('warga', where: 'nama LIKE ?', whereArgs: ['%$keyword%']);
  }
  ...
  Future<Map<String, dynamic>> getDashboardSummary() async {
    ...
    final lakiLaki   = await db.rawQuery("SELECT COUNT(*) as total FROM warga WHERE jenis_kelamin = 'Laki-laki'");
    final perempuan  = await db.rawQuery("SELECT COUNT(*) as total FROM warga WHERE jenis_kelamin = 'Perempuan'");
    ...
    return {
      ...
      'balita': balita, 'anak_anak': anakAnak,
  ...
  Future<List<Map<String, dynamic>>> getAllSPPT({String orderBy = 'nomor_petak ASC'}) async {
    final db = await database;
  ...
  Future<List<Map<String, dynamic>>> searchSPPTByNama(String keyword) async {
    final db = await database;
    return await db.query('sppt', where: 'nama_pemilik LIKE ?', whereArgs: ['%$keyword%']);
  }
